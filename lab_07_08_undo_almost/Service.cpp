...
Repo Service::create_repo_copy()
{
    ...
    for (int i = 0; i < size; i++)
    {
        int a = copied_ch[i].get_ziua();
        int b = copied_ch[i].get_suma();
        char *c = copied_ch[i].get_type();

        repo_copy.add_cheltuieli_zi_suma_type(a, b, c);
    }
    return repo_copy;
}
...
//    int size = repository.get_size();
//
//    Cheltuiala *repo_copy = new Cheltuiala[size];
//
//    for (int i = 0; i < size; i++)
//    {
//        int a = all_ch[i].get_ziua(), b = all_ch[i].get_suma();
//        char *c = all_ch[i].get_type();
//        Cheltuiala ch(a, b, c);
//        repo_copy[i] = ch;
//    }
//
